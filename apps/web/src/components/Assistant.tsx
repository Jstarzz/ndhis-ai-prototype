  ...
  return (
    <section className="work-panel assistant-workspace">
      ...
      <div className="chat-log" aria-live="polite">
        {messages.map((message, index) => <div className={`message ${message.role}`} key={`${message.role}-${index}`}><div className="message-label">{message.role === "assistant" ? "NDHIS AI" : "Doctor"}</div><div className="message-body"><p>{message.content}</p>{(message.tool || message.routing) && <details className="tool-trace"><summary>Request trace{message.tool ? ` · ${message.tool}` : ""}</summary><dl>{message.routing && <div><dt>Routing</dt><dd>{message.routing}</dd></div>}<div><dt>LLM calls</dt><dd>{message.llmCalls ?? "—"}</dd></div><div><dt>Latency</dt><dd>{message.latencyMs ?? "—"} ms</dd></div>{message.requestId && <div><dt>Request</dt><dd>{message.requestId}</dd></div>}{message.tool && <div><dt>Arguments</dt><dd><code>{JSON.stringify(message.arguments ?? {})}</code></dd></div>}</dl></details>}</div></div>)}
        {busy && <div className="message assistant"><div className="message-label">NDHIS AI</div><div className="message-body thinking">Checking the local route…</div></div>}
      </div>
