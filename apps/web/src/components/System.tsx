...
export function System() {
  ...
  return (
    <div className="system-layout">
      <section className="section-block"><div className="section-heading"><div><span className="section-label">Runtime</span><h2>Services and models</h2></div><button type="button" className="secondary small" onClick={load} disabled={loading}>Refresh</button></div>{error && <div className="error-box" role="alert"><strong>Runtime metadata unavailable</strong><span>{error}</span></div>}{loading && !info && <div className="empty-state"><strong>Loading runtime metadata</strong><span>Querying the gateway and local specialist services.</span></div>}{info && <><div className="runtime-facts"><div><span>Processing</span><strong>{info.processing}</strong></div><div><span>Profile</span><strong>{info.profile}</strong></div><div><span>Per-user limit</span><strong>{info.limits.requests_per_minute_per_user} RPM</strong></div><div><span>Concurrency</span><strong>{info.limits.max_concurrent_requests}</strong></div><div><span>Max request</span><strong>{Math.round(info.limits.max_body_bytes / 1024 / 1024)} MB</strong></div></div><div className="model-table">{Object.entries(info.models).map(([role, model]) => <div className="model-row" key={role}><span>{titleCase(role)}</span><strong>{model}</strong><Status state={info.services[serviceKeyForModel(role)] ?? (role === "asr" ? info.services.translation : "ready")} /></div>)}</div></>}</section>
      <section className="section-block"><div className="section-heading"><div><span className="section-label">Audit</span><h2>Recent execution events</h2></div><span className="muted">Browser view excludes user identity</span></div><AuditList events={audit} empty="No recent gateway events." /></section>
    </div>
  )
}
